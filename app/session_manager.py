...
def save_history(session_id:str, history:List[str]):
...
def save_jd(session_id: str, jd_text: str):
    r.set(f"jd:{session_id}", jd_text)

def get_jd(session_id: str) -> str:
    data = r.get(f"jd:{session_id}")
    return data.decode("utf-8") if data else None